...
class StanfordNLP:
    ...
    def parse(self, text):
        ...
        # print sentence
        try:
            url = config.SENTENCES_INFO_URL
            params = urllib.urlencode({'sentence': text})
            print params
            headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/json"}
            conn = httplib.HTTPConnection(config.SENTENCES_INFO_SERVER)
            conn.request('POST', url, params, headers)
            response = conn.getresponse()
            json = response.read()
            # print json
            json_data = demjson.decode(json)
            # return json_data
