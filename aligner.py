...
def wordRelatedness(word1, pos1, word2, pos2):
    if word1+pos1+word2+pos2 in tdict:
        return tdict[word1+pos1+word2+pos2 ]
    rvalue=wordRelatedness2(word1,pos1,word2,pos2)
    tdict[word1+pos1+word2+pos2]=rvalue
    return rvalue
...
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    for item in sourceNamedEntities:
        for jtem in item[1]:
            if item[3] in [u'PERSON', u'ORGANIZATION', u'LOCATION']:
    ...
    for item in sourceNamedEntities:
        if item[3] not in [u'PERSON', u'ORGANIZATION', u'LOCATION']:
            continue
        ...
        for jtem in targetNamedEntities:
            if jtem[3] not in [u'PERSON', u'ORGANIZATION', u'LOCATION']:
                continue
...
# ]
    ...
    for i in sourceWordIndices:
        # if (sourceLemmas[i-1] not in stopwords + punctuations + [u'\'s', u'\'d', u'\'ll']) or i in sourceWordIndicesAlreadyAligned:
        #     continue
        
        for j in targetWordIndices:
            # if (targetLemmas[j-1] not in stopwords + punctuations + [u'\'s', u'\'d', u'\'ll']) or j in targetWordIndicesAlreadyAligned:
            #     continue
...
def align(sentence1, sentence2):
    ...
    sentence1Lemmatized =[[s[0],s[1],s[2],s[2]] for s in sentence1PosTagged]
    sentence2Lemmatized = [[s[0],s[1],s[2],s[2]] for s in sentence2PosTagged]
    ...
    myWordAlignmentTokens = [[sentence1Lemmatized[item[0]-1][2], sentence2Lemmatized[item[1]-1][2]] for item in myWordAlignments]


    return [myWordAlignments, myWordAlignmentTokens,sentence1PosTagged,sentence2PosTagged]
